...
sub AUTOLOAD {
    ...
    unless (blessed($l)) {
         longmess "OLog invoked with an unblessed reference??";
    }
    unless ($l->{_log}) {
        $l->{_log} = get_logger($l);
    }
...
sub get_log_file {
    return OLED->config->{'Log4perlConfig'} || '/etc/emb-oled/log4perl.conf';
}

sub get_logger {
    initLogger() unless Log::Log4perl->initialized();
    my $l = Log::Log4perl->get_logger();
    $l->level(_levelToLog4perlLevelInt($ENV{OLED_LOG_LEVEL})) if $ENV{OLED_LOG_LEVEL};
    return $l;
}

sub initLogger {
    my $l4pf = get_log_file();
...
#sleep 1;
    ...
    if ($ENV{HEATER_TEST_MODE}) {
        Log::Log4perl->easy_init({level => _levelToLog4perlLevelInt($ENV{OLED_LOG_LEVEL} || 'TRACE'),
                                  utf8 => 1,
                                 });
    } else {
...
sub _levelToLog4perlLevelInt {
    ...
    die "_levelToLog4perlLevelInt($_[0]):> Unknown log level $_[0].".($ENV{OLED_LOG_LEVEL} ? " Log level set in \$ENV{OLED_LOG_LEVEL} => '$ENV{OLED_LOG_LEVEL}'" : "");
}
